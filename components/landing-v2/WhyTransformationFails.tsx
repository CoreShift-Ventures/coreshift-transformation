...
export function WhyTransformationFails() {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          className={`rounded-2xl p-6 border ${
            isDark ? 'bg-gray-900 border-[#ec5f2b]/30' : 'bg-gray-50 border-[#ec5f2b]/30'
          }`}
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          <h3 className={`text-lg font-bold text-center mb-6 ${isDark ? 'text-gray-100' : 'text-brand-charcoal'}`}>
            The <span className="text-[#ec5f2b]">CoreShift</span> Difference
          </h3>

          <div className="grid md:grid-cols-3 gap-6 text-center">
            {[
              { title: 'Strategy + Execution', desc: "We don't just consult, we build & deliver" },
              { title: 'Process First, Then Tech', desc: 'Transform the process, then automate with AI' },
              { title: 'You Own Everything', desc: 'Your infrastructure, zero vendor lock-in' }
            ].map((item, idx) => (
              <motion.div
                key={idx}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <div className={`text-base font-bold mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                  {item.title}
                </div>
