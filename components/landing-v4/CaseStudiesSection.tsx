      ...
      <div className="hidden lg:block max-w-7xl mx-auto px-6">
        ...
        <motion.div
          ...
          className={`rounded-2xl p-8 md:p-10 relative overflow-hidden ${
            isDark
              ? 'bg-gradient-to-r from-gray-900 via-gray-900 to-gray-800 border border-gray-800'
              : 'bg-gradient-to-r from-gray-50 via-white to-gray-50 border border-gray-200'
          }`}
        >
          {/* Background accent */}
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#ec5f2b] opacity-5 rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-1/4 w-48 h-48 bg-[#ec5f2b] opacity-5 rounded-full blur-3xl" />
          ...
          <div className="relative flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="text-center md:text-left">
              ...
              <p className={`text-sm md:text-base max-w-xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                That's the point. We've never met an operation we couldn't agent-ify. Tell us what your team does manually, and we'll show you what an agent for it would look like.
              </p>
            </div>
            <a
...
function AgentCard({ agent, isDark, index }: { agent: typeof agents[0], isDark: boolean, index: number }) {
  return (
    ...
    >
      {/* Top accent bar */}
      <div className={`h-1 ${agent.statusColor === 'green' ? 'bg-gradient-to-r from-green-500 to-emerald-400' : 'bg-gradient-to-r from-amber-500 to-yellow-400'}`} />
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <span className={`relative flex h-2 w-2`}>
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${agent.statusColor === 'green' ? 'bg-green-500' : 'bg-amber-500'}`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${agent.statusColor === 'green' ? 'bg-green-500' : 'bg-amber-500'}`}></span>
            </span>
          ...
          <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${isDark ? 'bg-gray-800 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>
            {agent.industry}
          </span>
        </div>
      ...
      <div className="px-5 py-4">
        <p className={`text-xs leading-relaxed mb-4 line-clamp-3 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          {agent.tagline}
        </p>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          {agent.metrics.map((metric, idx) => (
            <div key={idx} className={`relative pl-3 py-2 rounded-lg ${isDark ? 'bg-gray-800/60' : 'bg-gray-50'}`}>
              <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-lg bg-[#ec5f2b]" />
        ...
        <div className={`pt-3 border-t ${isDark ? 'border-gray-800' : 'border-gray-100'}`}>
          <div className={`rounded-lg p-3 ${isDark ? 'bg-gray-800/40' : 'bg-gray-50/80'}`}>
            <p className={`text-xs italic leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
              "{agent.quote}"
            </p>
            <p className={`text-[10px] mt-2 font-semibold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>
              {agent.client}
            </p>
          </div>
        </div>
